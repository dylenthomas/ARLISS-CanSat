...
static bool vlened_assigned = false;
...
void storeValues(unsigned char* buffer, int& msg_len, int& msg_idx) {
    ...
    switch (msg_idx) {
        case 0:
            memcpy(&posllhMessage, trimmed_buff, sizeof(POSLLH));
            posllh_assigned = true;
            break;
        case 1:
            memcpy(&statusMessage, trimmed_buff, sizeof(STATUS));
            status_assigned = true;
            break;
        case 2:
            memcpy(&velnedMessage, trimmed_buff, sizeof(VELNED));
...
posllhData getPOSLLH() {
    ...
    posllh_data.hACC = (float)(posllhMesssage.hACC / pow(10, 3));
...
statusData getSTATUS() {
    ...
    status-data.iTOW = statusMessage.iTOW;
...
velnedData getVELNED() {
    ...
    velned_data.id = velenedMessage.id;
    velned_data.len = velnedMessage.len;
    velned_data.iTOW = velnedMessage.iTOW;
    velned_data.velN = (double)(velnedMessage.velN / pow(10, 2));
    velned_data.velE = (double)(velnedMessage.velE / pow(10, ));
    velned_data.velD = (double)(velnedMessage.velD / pow(10, ));
    velned_data.speed = (double)(velnedMessage.speed / pow(10, ));
    velned_data.gSpeed = (double)(velnedMessage.gSpeed / pow(10, ));
    velned_data.heading = (float)(velnedMessage.heading / pow(10, ));
    velned_data.sAcc = (float)(velnedMessage.sAcc / pow(10, ));
    velned_data.cAcc = (float)(velnedMessage.cAcc / pow(10, ));
