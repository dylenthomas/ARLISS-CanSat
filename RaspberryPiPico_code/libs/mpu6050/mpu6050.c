...
void mpu6050_ping() {
    ...
    handle_err("ping write", i2c_write_blocking(i2c_dev, mpu_addr, &who_am_i_reg, 1, true));
...
void mpu6050_wake(bool rst_to_default) {
    uint8_t wake[2] = {pwr_mgmt_1_reg, 0x00};
    if (rst_to_default) {
        wake[1] = 0x80;
    }
...
void mpu6050_configure(int gyro_range, int accel_range) {
    if (gyro_range < 0 || gyro_range > 3) {
        handle_err("gyro range", -1);
    }
    if (accel_range < 0 || accel_range > 3) {
        handle_err("accel_range", -1);
    }
    ...
    uint8_t gyro_conf[2] = {gyro_conf_reg, (gyro_range & 0x03) << 3};
    uint8_t accel_conf[2] = {accel_conf_reg, (accel_range & 0x03) << 3};
...
void mpu6050_setlowpwr() {
    uint8_t pwr1_conf[2] = {pwr_mgmt_1_reg, 0x28};
    uint8_t pwr2_conf[2] = {pwr_mgmt_2_reg, 0x7F};

    handle_err("low power1 write", i2c_write_blocking(i2c_dev, mpu_addr, pwr1_conf, 2, false));
    handle_err("low power2 write", i2c_write_blocking(i2c_dev, mpu_addr, pwr2_conf, 2, false));
}

void mpu6050_setfullpwr() {
    uint8_t pwr1_conf[2] = {pwr_mgmt_1_reg, 0x00};

    handle_err("full power write", i2c_write_blocking(i2c_dev, mpu_addr, pwr1_conf, 2, false));
}
...
void mpu6050_read(float accel[3], float gyro[3], float* temp) {
    ...
    handle_err("all data write", w_result);

    if (w_result > 0) {
        r_result = i2c_read_blocking(i2c_dev, mpu_addr, buffer, 14, false);
        handle_err("all data read", r_result);
    }
